...
export class ButtonHandler extends InteractionHandler {
    ...
    public async run(interaction: ButtonInteraction) {
        ...
        if (permited) {
            ...
            await this.container.prisma.userNotes.deleteMany({
                where: {
                    user_id: userId,
                    guild_id: guildId
                }
            ...
            const this_guild_config_channels = await this.container.prisma.configChannels.findUnique({
                where: {
                    guild_id: guildId
                }
            })
          ...
          **Usuario:** ${member.user.username}${member.user.discriminator && parseInt(member.user.discriminator) != 0 ? `#${member.user.discriminator}` : ""} (${userId})
          `
                ...
                const notes_logs_channel: any = this.container.client.channels.resolve(this_guild_config_channels.notes_logs)
                notes_logs_channel
